...
if ( ! class_exists( 'HFE_Addons_Actions' ) ) {
	...
	class HFE_Addons_Actions {
		...
		public function get_api_domain() {
			return apply_filters( 'hfe_api_domain', 'https://themephi.net/' );
		}
			...
			if ( isset( $_POST['plugin'] ) ) {
				...
				if ( 'plugin' === $type ) {
					...
					if ( ! current_user_can( 'activate_plugins' ) ) {
						wp_send_json_error( esc_html__( 'Plugin activation is disabled for you on this site.', 'themephi-header-footer-elementor' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						wp_send_json_success( esc_html__( 'Plugin Activated.', 'themephi-header-footer-elementor' ) );
					}
				}
				...
				if ( 'theme' === $type ) {
					...
					if ( ! ( current_user_can( 'switch_themes' ) ) ) {
						wp_send_json_error( esc_html__( 'Theme activation is disabled for you on this site.', 'themephi-header-footer-elementor' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						wp_send_json_success( esc_html__( 'Theme Activated.', 'themephi-header-footer-elementor' ) );
					}
				}
			}

			if ( 'plugin' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate plugin. Please activate from the Plugins page.', 'themephi-header-footer-elementor' ) );
			} elseif ( 'theme' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate theme. Please activate from the Themes page.', 'themephi-header-footer-elementor' ) );
			}
		}
