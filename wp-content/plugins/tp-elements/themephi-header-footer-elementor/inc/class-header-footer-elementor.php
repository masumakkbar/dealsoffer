...
class Header_Footer_Elementor {
	...
	function __construct() {
		...
		if ( $is_elementor_callable ) {
			...
			if ( 'yes' === get_option( 'themephi_hfe_plugin_is_activated' ) ) {
	...
	public function elementor_not_installed_activated() {
		...
		if ( ! did_action( 'elementor/loaded' ) ) {
			...
			$message = sprintf( __( 'The %1$sElementor Header & Footer Builder%2$s plugin requires %1$sElementor%2$s plugin installed & activated.', 'themephi-header-footer-elementor' ), '<strong>', '</strong>' );
			...
			if ( _is_elementor_installed() ) {
				...
				$button_label = __( 'Activate Elementor', 'themephi-header-footer-elementor' );

			} else {

				$action_url   = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=elementor' ), 'install-plugin_elementor' );
				$button_label = __( 'Install Elementor', 'themephi-header-footer-elementor' );
			}
	...
	public function elementor_outdated() {
		...
		$class = 'notice notice-error';
		/* translators: %s: html tags */
		$message = sprintf( __( 'The %1$sElementor Header & Footer Builder%2$s plugin has stopped working because you are using an older version of %1$sElementor%2$s plugin.', 'themephi-header-footer-elementor' ), '<strong>', '</strong>' );
		...
		if ( file_exists( WP_PLUGIN_DIR . '/elementor/elementor.php' ) ) {
			...
			$button_label = __( 'Update Elementor', 'themephi-header-footer-elementor' );
	...
	public function show_setup_wizard() {
		...
		/* translators: %s: html tags */
		$notice_message = sprintf( __( 'Thank you for installing %1$s Elementor Header & Footer Builder %2$s Plugin! Click here to %3$sget started. %4$s', 'themephi-header-footer-elementor' ), '<strong>', '</strong>', '<a href="' . $setting_url . '">', '</a>' );
	}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'themephi-header-footer-elementor' );
	}
	...
	public function setup_unsupported_theme( $hfe_settings_tabs = [] ) {
		if ( ! current_theme_supports( 'themephi-header-footer-elementor' ) ) {
			$hfe_settings_tabs['hfe_settings'] = [
				'name' => __( 'Theme Support', 'themephi-header-footer-elementor' ),
