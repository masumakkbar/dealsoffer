<?php
if( !class_exists( 'Couponis_Social_Login' ) ){
class Couponis_Social_Login{

	public $networks;
	public $social_enabled;

	public function __construct(){
		add_action('couponis_social_login_button', array( $this, 'show_buttons' ) );
		add_action('wp_enqueue_scripts', array( $this, 'load_scripts' ), 12 );

		$this->networks = array(
			'facebook' 	=> array(
				'title' 	=> esc_html__( 'Facebook', 'dealsoffer' ),
				'data'		=> array()
			),
			'twitter'	=> array(
				'title'		=> esc_html__( 'Twitter', 'dealsoffer' ),
				'data'		=> array()
			),
			'google'	=> array(
				'title'		=> esc_html__( 'Google', 'dealsoffer' ),
				'data'		=> array()
			)			
		);	
	...
	public function start_social_login(){
		...
		foreach( $this->networks as $network => &$data ){
			$id = $dealsoffer_option[$network.'_app_id'];
			$secret = $dealsoffer_option[$network.'_app_secret'];
			$data['data'] = array(
				'id'		=> $id,
				'secret'	=> $secret
			);
		...
		if( !empty( $_GET['couponis-connect'] ) ){
			switch( $_GET['couponis-connect'] ){
	...
	public function callback(){
		if( !empty( $_GET['couponis-callback'] ) ){
			switch( $_GET['couponis-callback'] ){
	...
	public function load_scripts(){
		wp_localize_script( 'couponis-custom', 'couponis_sc', array(
			'facebook' 	=> site_url( 'index.php?dealsoffer-connect=facebook' ),
			'twitter' 	=> site_url( 'index.php?dealsoffer-connect=twitter' ),
			'google' 	=> site_url( 'index.php?dealsoffer-connect=google' )
		));
	}
	...
	public function show_buttons(){
		if( $this->social_enabled == true ){
			...
			<div class="text-center">
				<label><?php esc_html_e( 'Sign In With', 'dealsoffer' ) ?></label>
			</div>
	...
	private function _login_user( $userdata ){
		...
		if( !empty( $user_id ) ){
			update_user_meta( $user_id, 'couponis_avatar_url', $userdata['user_avatar'] );
		}
	...
	public function facebook_connect(){
		wp_redirect( 'https://graph.facebook.com/oauth/authorize?scope=email&client_id='.esc_attr( $this->networks['facebook']['data']['id'] ).'&redirect_uri='.site_url('index.php?dealsoffer-callback=facebook') );
		die();
	}
	...
	public function facebook_callback(){
		if( isset( $_GET['code'] ) ){
			$response = $this->_http_facebook( 'oauth/access_token', array(
				...
				'redirect_uri'	=> site_url( 'index.php?dealsoffer-callback=facebook' ),
			...
			if( !empty( $response->access_token ) ){
				...
				if( !empty( $user->id ) ){
					...
					esc_html_e( 'Could not retrieve user data', 'dealsoffer' );
				}
			}
			else{
				esc_html_e( 'Could not generate access_token', 'dealsoffer' );
			}
		} 
		else {
	...
	public function twitter_connect(){
		$response = $this->_http_twitter( 'oauth/request_token', array(
			'oauth_callback' => site_url('/index.php?dealsoffer-callback=twitter')
		));
		...
		if( !empty( $response['oauth_callback_confirmed'] ) && $response['oauth_callback_confirmed'] == true ){
			...
			esc_html_e( 'Could not generate oauth_token since credentials are invalid', 'dealsoffer' );
		}
		die();
	}
	...
	public function twitter_callback(){
		if( isset( $_GET['oauth_token'] ) && isset( $_GET['oauth_verifier'] ) ){
			...
			if( !empty( $response['oauth_token'] ) && !empty( $response['oauth_token_secret'] ) ){
				...
				esc_html_e( 'Could not generate oauth_token_secret since credentials are invalid', 'dealsoffer' );
			}

		} 
		else {
			esc_html_e( 'Login error', 'dealsoffer' );
		}
		die();
	}
	...
	public function google_connect(){
		$data = array(
			...
			'redirect_uri'		=> site_url('index.php?dealsoffer-callback=google'),
	...
	public function google_callback(){
		if( isset( $_GET['code'] ) ){
			...
			if( !empty( $response->access_token ) ){
				...
				esc_html_e( 'Could not retrieve access_token', 'dealsoffer' );	
			}
		} 
		else {
			esc_html_e( 'Login error', 'dealsoffer' );
...
$couponis_login = new Couponis_Social_Login();
}
